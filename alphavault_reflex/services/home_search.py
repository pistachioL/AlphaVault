...
def search_stocks(
    query: str, *, limit: int = _DEFAULT_STOCK_RESULT_LIMIT
) -> StockSearchResult:
    clean_query = _clean_text(query)
    if not clean_query:
        return {"exact_href": "", "rows": [], "error": ""}
    try:
        ...
        if len(exact_keys) == 1:
            exact_href = build_stock_route(next(iter(exact_keys)))
            return {
                "exact_href": exact_href,
                "rows": _rows_to_search_rows(exact_rows, exact_keys=exact_keys)[
                    : max(1, int(limit))
                ],
                "error": "",
            }
        ...
        return {"exact_href": "", "rows": [], "error": str(exc)}

    merged_rows = _rows_to_search_rows(
