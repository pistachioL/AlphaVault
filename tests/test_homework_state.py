...
def test_homework_state_refresh_default_view_uses_live_loader(monkeypatch) -> None:
    calls: list[str] = []
    ...
    def _fake_load_trade_feed(*, view_key="default"):  # type: ignore[no-untyped-def]
        calls.append(f"feed:{view_key}")
        return {}

    def _fake_load_board_payload(lookback_days):  # type: ignore[no-untyped-def]
        calls.append(f"live:{lookback_days}")
        return assertions, [], ""
    ...
    assert calls == [f"live:{TRADE_BOARD_DEFAULT_WINDOW_DAYS}"]
    assert len(state.rows) == 1
    assert save_calls == []
