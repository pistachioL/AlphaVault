...
def test_sync_stock_hot_cache_discards_stock_dirty_entries_without_rebuild(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _conn, **kwargs: hot_calls.append(str(kwargs.get("stock_key") or "")),
    )
    monkeypatch.setattr(
        stock_hot_cache,
...
def test_sync_stock_hot_cache_skips_bootstrap_when_dirty_queue_is_empty(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        stock_hot_cache,
    ...
    assert int(stats.get("processed", 0)) == 0
    assert int(stats.get("written", 0)) == 0
    assert bool(stats.get("has_more", True)) is False
...
def test_sync_stock_hot_cache_yields_to_rss_after_current_stock(monkeypatch) -> None:
    ...
    assert removed == [(["cluster:1"], "2026-04-04 10:10:00+08:00")]
    assert released == [(["cluster:2"], "2026-04-04 10:10:00+08:00")]
...
def test_sync_stock_hot_cache_marks_claim_failed_when_refresh_raises(
    ...
    assert failed == [(["cluster:white_liquor"], "2026-04-04 10:10:00+08:00")]
