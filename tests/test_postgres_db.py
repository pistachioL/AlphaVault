...
def test_dispose_cached_postgres_engines_closes_cached_pools(monkeypatch) -> None:
    from alphavault.db import postgres_db
    ...
    postgres_db._dispose_cached_postgres_engines()

    assert len(created_pools) == 2
    assert getattr(created_pools[0], "closed", False) is True
    assert getattr(created_pools[1], "closed", False) is True
